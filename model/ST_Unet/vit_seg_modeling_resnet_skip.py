...
from model.ST_Unet.deform_conv import DeformConv2d
...
ATTENTION_V = "MultiHeadDotProdufctAttention_1/value"
...
class PreActBottleneck(nn.Module):
    ...
    def __init__(self, cin, cout=None, cmid=None, stride=1):
        super().__init__()
        cout = cout or cin  # 256(3) 512(4) 1024(9) #判断是否为真
        cmid = cmid or cout // 4  # 64(3)  128(4) 256(9)

        self.gn1 = nn.GroupNorm(32, cmid, eps=1e-6)  # 将channel切分成许多组进行归一化
    ...
    def forward(self, x):
        ...
        y = self.relu(residual + y)  # 构建残差网络
        return y

    def load_from(self, weights, n_block, n_unit):
        # pjoin路径拼接函数，将相应的参数拼接
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:  # 没有进行相应的归一化操作
            x = self.norm(x)
        return x
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # 定义生成参数表格 [(2m-1)*(2m-1),num_heads]维度，的索引表，需要学习
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH

        # 在窗口中的相对位置索引
        coords_h = torch.arange(self.window_size[0])  # torch.arange以0为起点的（0-7）
        coords_w = torch.arange(self.window_size[1])
        coords = torch.stack(torch.meshgrid([coords_h, coords_w]))  # 2, Wh, Ww # meshgrid生成网格坐标
        # 生成了相对位置信息（-2，4）这种
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)] \
            .view(self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1],
                  -1)  # Wh*Ww,Wh*Ww,nH
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim,
        ...
        self.num_heads = num_heads  # 注意力头的数目
        self.window_size = window_size  # 窗口大小
        self.shift_size = shift_size  # 移动的尺寸
        ...
        self.norm1 = norm_layer(dim)  # 进行归一化操作
        ...
        if self.shift_size > 0:
            ...
            # 进行窗口的切分，（0到-8）（-8到shift_size）（shift_size到0）
            # 也就是切块，切割成为4块
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        # DeformConv2d可变形卷积
        # 基于在模块中增加额外偏移量的空间采样位置和从目标任务中学习到偏移量且不需要额外的监督
        # 每次卷积后的通道数会减少一半
        ...
        self.avgpool = nn.AdaptiveAvgPool1d(1)  # 一维池化操作
        self.conv11 = conv1x1(dim, dim // 2, bias=False)  # 1*1卷积，用于改变通道数
        self.gelu = nn.GELU()
        self.pool_w, self.pool_h = nn.AdaptiveAvgPool2d((1, None)), nn.AdaptiveAvgPool2d((None, 1))  # 池化，将图像池化为固定尺寸
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            ...
            if i % 2 == 0:
                ...
                s = torch.matmul(s_h, s_w)  # 将相应的图像进行还原

                s = self.gelu(self.bn2(self.conv2(s)))

            # 到这一步才是Swin tranformer的部分
            x = blk(x)
            i = i + 1
...
class ChannelAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        self.pool = nn.MaxPool2d(kernel_size=(x.size(2), x.size(3)), stride=(x.size(2), x.size(3)))
...
class TransResNetV2(nn.Module):
    ...
    def __init__(self, config, block_units, width_factor):
        ...
        # OrderedDict按相应顺序进行储存
        self.root = nn.Sequential(OrderedDict([
            ('conv', StdConv2d(3, width, kernel_size=7, stride=2, bias=False, padding=3)),  # 进行相应的卷积操作
        ...
        depths = config.trans.depths  # [2,2,6,2]transformer的网络深度
        ...
        patches_resolution = self.embeddings.patches_resolution  # [64,64]
        self.patches_resolution = patches_resolution
        ...
        # 根据相应的transformer的层数来构建相应的网络结构
        # 根据论文中的要求分为4层结构
        for i_layer in range(self.num_layers):  # i_layer 0,1,2,3
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),  # 相应的输出维数
                               input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                 patches_resolution[1] // (2 ** i_layer)),
                               depth=depths[i_layer],  # 网络深度
                               num_heads=config.trans.num_heads[i_layer],  # transformer注意力机制头的数目
                               window_size=config.trans.window_size,  # 窗口大小为8
                               mlp_ratio=self.mlp_ratio,  # mlp的比率值
                               qkv_bias=qkv_bias, qk_scale=qk_scale,
                               drop=drop_rate, attn_drop=attn_drop_rate,  # 相应的权重偏差
                               drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])],  # 提取相应的参数
                               norm_layer=norm_layer,
                               no_down=i_layer,
