...
from model.ST_Unet import vit_seg_configs as configs
from model.ST_Unet.vit_seg_modeling_resnet_skip import TransResNetV2
from model.ST_Unet.model_resnet import *
...
class Transformer(nn.Module):
    def __init__(self, config, img_size):
        ...
        img_size = _pair(img_size)  # 构建（256，256）的数组
        # print(img_size)
        self.hybrid_model = TransResNetV2(config, block_units=config.resnet.num_layers,
                                          width_factor=config.resnet.width_factor)  # 建立混合transformer和cnn的模型
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=256, num_classes=6, zero_head=False):
        ...
        self.zero_head = zero_head  # 头的数量

        self.classifier = config.classifier
        self.transformer = Transformer(config, img_size)  # 构造transformer和cnn的混合函数
